...
2. **UXML / USS の GUID プレースホルダーを実際の GUID に置き換えたか？**
3. **ルート要素に `dennoko-root` クラスを付けたか？**
   - 付け忘れると変数が解決されず、全要素がエディタテーマの見た目に戻る。
4. **C# で `new VisualElement()` を手組みしてスタイリングしていないか？**
   - 動的リスト以外のレイアウトは UXML へ。色指定は USS 変数へ。
5. **Inspector の場合、`Bind(serializedObject)` を呼んだか？**
6. **ウィンドウを極端に狭く / 広くしてレイアウト崩れがないか？**
